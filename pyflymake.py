...
def log(msg):
    pass
    # with open('/Users/erik.allik/debug.txt', 'wa') as f:
    #     print >> f, msg
...
class LintRunner(object):
    ...
    def run(self, filename):
        log("%s.run(%s)" % (type(self).__name__, repr(filename)))
...
class PyflakesRunner(LintRunner):
    ...
    def run(self, filename):
        ...
        for line in getattr(process, 'stdout'):
            log("line: %s" % repr(line))
            self.process_output(line)
        for line in getattr(process, 'stderr'):
            log("line: %s" % repr(line))
            self.process_output(line)
